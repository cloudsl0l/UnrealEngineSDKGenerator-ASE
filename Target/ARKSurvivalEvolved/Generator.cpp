...
public:
	bool Initialize(void* module) override
	{
		...
		virtualFunctionPattern["Class CoreUObject.Object"] = {
			// UObject::ProcessEvent prologue (vtable index 60 in ShooterGame.pdb)
			{ "\x40\x55\x53\x56\x57\x41\x54\x41\x56\x41\x57\x48\x81\xEC\xF0\x00\x00\x00\x48\x8D\x6C\x24\x30", "xxxxxxxxxxxxxxxxxxxxxxx", 0x30, R"(	inline void ProcessEvent(class UFunction* function, void* parms)
	{
			...
			PredefinedMethod::Default("bool IsA(UClass* cmp) const", R"(bool UObject::IsA(UClass* cmp) const
{
	...
	return false;
		...
		predefinedMethods["Class CoreUObject.Class"] = {
			// UClass::CreateDefaultObject is not virtual in this build; read ClassDefaultObject (0xF8) instead
			PredefinedMethod::Inline(R"(	inline UObject* GetDefaultObject() const
	{
		return *reinterpret_cast<UObject* const*>(reinterpret_cast<uintptr_t>(this) + 0xF8);
	})"),
			PredefinedMethod::Inline(R"(	template<typename T>
...
public:
	...
	{
		const auto item = GetObjectPtr(index);
		return item != nullptr ? item->Object : nullptr;
	}

	inline UObject* operator[](int32_t index) const
	{
